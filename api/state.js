import { configured, getLatestState, getLatestPrices, getLatestRunSummary } from './_lib/db.js'
...
export default async function handler(req, res) {
  try {
    ...
    const [state, prices, run] = await Promise.all([getLatestState(), getLatestPrices(), getLatestRunSummary()])
    // Cache at the edge for a minute — the ingest runs at most daily.
    res.setHeader('Cache-Control', 's-maxage=60, stale-while-revalidate=600')
    return res
      .status(200)
      .json({ configured: true, ...(state || { empty: true }), prices: prices ?? undefined, run: run ?? undefined })
  } catch (err) {

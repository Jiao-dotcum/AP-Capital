import { configured, getLatestState, getLatestPrices } from './_lib/db.js'
...
// so the client falls back to factor-modeled marks.
export default async function handler(req, res) {
  try {
    if (!configured()) return res.status(200).json({ configured: false })
    const [state, prices] = await Promise.all([getLatestState(), getLatestPrices()])
    // Cache at the edge for a minute — the ingest runs at most daily.
    res.setHeader('Cache-Control', 's-maxage=60, stale-while-revalidate=600')
    return res.status(200).json({ configured: true, ...(state || { empty: true }), prices: prices ?? undefined })
  } catch (err) {
